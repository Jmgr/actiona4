...
On Windows, install OpenCV locally (MSVC x64 build). If OpenCV is not discoverable automatically, set `OpenCV_DIR` or `OPENCV_LINK_PATHS`/`OPENCV_LINK_LIBS` so the `opencv` crate can link.
`cargo test` will copy the matching `opencv_world*.dll` from that installation into `target/.../deps` automatically.
