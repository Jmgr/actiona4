use action_definition::{actions::RandomNumber, post_run::PostRun};

use crate::{ExecutionContext, ResolveParam, RunError, RunErrorKind, Runnable};

impl Runnable for RandomNumber {
    async fn run(&self, context: &mut ExecutionContext) -> Result<PostRun, RunError> {
        ...
        if minimum >= maximum {
            return Err(RunError::new(RunErrorKind::InvalidRandomNumberRange {
                minimum,
                maximum,
            }));
        }
