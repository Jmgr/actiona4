...
mod tests {
    ...
    fn test_mode() {
        test_with_file(|script_engine| async move {
            ...
            #[cfg(windows)]
            {
                let mode_error = script_engine
                    .eval_async::<()>("await file.mode()")
                    .await
                    .unwrap_err();
                assert_eq!(
                    mode_error.to_string(),
                    "Error: Unsupported on this platform: not supported on Windows"
                );

                let set_mode_error = script_engine
                    .eval_async::<()>("await file.setMode(0o445)")
                    .await
                    .unwrap_err();
                assert_eq!(
                    set_mode_error.to_string(),
                    "Error: Unsupported on this platform: not supported on Windows"
                );
            }
        });
