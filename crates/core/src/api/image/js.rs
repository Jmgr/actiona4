...
mod tests {
    ...
    fn test_generate(#[case] operation: String) {
        Runtime::test_with_script_engine(async move |script_engine| {
            ...
            let input_path = test_data_dir.join("Crown_icon_transparent.png");
            let overlay_path = test_data_dir.join("250px-Search-icon-transparent-background.png");
    ...
    fn test_find() {
        Runtime::test_with_script_engine(async move |script_engine| {
            ...
            // With mpsc every sent stage is guaranteed delivered — assert the full sequence.
            assert_eq!(
                result.first().map(String::as_str),
                Some("Downscaling"),
                "first stage should be Downscaling"
            );
            assert!(
                result.contains(&"Matching".to_string()),
                "Matching stage should appear"
            );
            assert!(
                result.contains(&"Filtering".to_string()),
                "Filtering stage should appear"
            );
            assert!(
                result.contains(&"ComputingResults".to_string()),
                "ComputingResults stage should appear"
            );
            assert_eq!(
                result.last().map(String::as_str),
                Some("Finished"),
                "last stage should be Finished"
            );
        })
    }
    ...
    fn test_find_all() {
        Runtime::test_with_script_engine(async move |script_engine| {
            ...
            assert!(
                stages.contains(&"Matching".to_string()),
                "Matching stage should appear"
            );
            assert!(
                stages.contains(&"Filtering".to_string()),
                "Filtering stage should appear"
            );
            assert!(
                stages.contains(&"ComputingResults".to_string()),
                "ComputingResults stage should appear"
            );
            assert_eq!(
    ...
    #[test]
    #[serial]
    fn test_find_all_without_colors_matches_grayscale_distractor() {
        Runtime::test_with_script_engine(async move |script_engine| {
            let test_data_dir = test_data_dir();
            let source_path = test_data_dir.join("input.png");
            let template_path = test_data_dir.join("Crown_icon_transparent.png");
            ...
            let result = script_engine
                .eval_async::<bool>(
                    ...
                    results.length === 3
                        && results.some(result =>
                            result.position.equals(new Point(767, 252))
                        )
                    "#,
                )
                .await
