...
/// Options for `waitForMovement`.
#[options]
#[derive(Clone, Debug, FromJsObject)]
pub struct JsWaitForMovementOptions {
    /// Abort signal to cancel the wait.
    pub signal: Option<JsAbortSignal>,
}
...
impl JsMouse {
    ...
    ///
    /// ```ts
    /// // Wait for any mouse movement
    /// const position = await mouse.waitForMovement();
    /// console.println(`Mouse moved to ${position}`);
    /// ```
    ///
    /// ```ts
    /// // Wait for movement with abort support
    /// const controller = new AbortController();
    /// const position = await mouse.waitForMovement({ signal: controller.signal });
    /// ```
    /// @param options?: WaitForMovementOptions
    /// @returns Task<Point>
    #[platform(not = "wayland")]
    pub fn wait_for_movement<'js>(
        &self,
        ctx: Ctx<'js>,
        options: Opt<JsWaitForMovementOptions>,
    ) -> Result<Promise<'js>> {
        let options = options.0.unwrap_or_default();
        let local_mouse = self.inner.clone();

        task_with_token(ctx, options.signal, async move |ctx, token| {
            let position = local_mouse
                .wait_for_movement(token)
                .await
                .into_js_result(&ctx)?;
            Ok(JsPoint::from(position))
        })
    }
    ...
    fn test_wait_for_movement() {
        Runtime::test_with_script_engine(async |script_engine| {
            _ = script_engine
                .eval_async::<()>(
                    r#"
                    console.println("Move the mouse...");
                    const position = await mouse.waitForMovement();
                    console.println(`Mouse moved to ${position}`);
                "#,
                )
