...
pub fn capture_rect(rect: Rect) -> Result<Vec<u8>> {
    ...
    // Capture the result but defer cleanup so GDI handles are always released,
    // even if BitBlt or GetDIBits fails.
    ...
    // Always release GDI resources regardless of whether the capture succeeded.
