...
mod tests {
    ...
    fn test_download_progress() {
        Runtime::test(async move |runtime| {
            ...
            let web = Web::new(runtime.task_tracker());
            let (sender, mut receiver) = mpsc::unbounded_channel::<Progress>();
            ...
            let receiver_handle = runtime.task_tracker().spawn(async move {
                while let Some(value) = receiver.recv().await {
                    local_received_progress.lock().push(value);
                }
            });
            ...
            // With mpsc every sent value is guaranteed delivered — assert the full sequence.
            assert!(
                matches!(received_progress.first(), Some(Progress::Uploading { .. })),
                "first progress event should be Uploading"
            );
            assert!(
                received_progress
                    .iter()
                    .any(|p| matches!(p, Progress::Downloading { .. })),
                "Downloading progress should appear"
            );
            assert_eq!(
                received_progress.last(),
                Some(&Progress::Finished),
                "last progress event should be Finished"
            );
            assert!(
                !received_progress.contains(&Progress::Inactive),
                "Inactive should never appear in progress updates"
            );
            // Downloading current values should be non-decreasing
            let downloading_currents: Vec<u64> = received_progress
                .iter()
                .filter_map(|p| {
                    if let Progress::Downloading { current, .. } = p {
                        Some(*current)
                    } else {
                        None
                    }
                ...
                .collect_vec();
            assert!(
                downloading_currents.windows(2).all(|w| w[0] <= w[1]),
                "downloading current values should be non-decreasing: {downloading_currents:?}"
            );
        });
    }
