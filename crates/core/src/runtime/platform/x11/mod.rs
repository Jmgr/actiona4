...
fn keyboard_text_event(
    text: String,
    key: enigo::Key,
    scan_code: u32,
    is_injected: bool,
    is_repeat: bool,
) -> Option<KeyboardTextEvent> {
    (!text.is_empty() && !text.chars().all(char::is_control)).then_some(KeyboardTextEvent {
        text,
        key,
        scan_code,
        is_injected,
            ...
            loop {
                ...
                match event {
                    ...
                    Event::XinputRawMotion(event) => {
                        let is_injected = input_devices.is_xtest_pointer(event.sourceid);
                        let reply = connection
                            .query_pointer(local_x11_connection.screen().root)
                            .await?
                            .reply()
                            .await?;
                    ...
                    Event::XinputRawKeyPress(event) => {
                        ...
                        let is_injected = input_devices.is_xtest_keyboard(event.sourceid);

                        keyboard_state.update_key(keycode, KeyDirection::Down);

                        let text_event = keyboard_text_event(
                            name.clone(),
                            key,
                            keycode.into(),
                            is_injected,
