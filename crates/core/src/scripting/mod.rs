...
impl Engine {
    ...
    fn process_caught_result<T>(
        ...
        result.map_err(|error| Self::process_caught_error(error, sourcemaps))
    }

    fn process_caught_error(
        error: CaughtError<'_>,
        sourcemaps: &Mutex<HashMap<u64, TsToJs>>,
    ) -> ScriptError {
        match error {
            CaughtError::Error(error) => ScriptError::quickjs(&error),
            CaughtError::Exception(exception) => {
                let processed = Self::process_exception(&exception, sourcemaps);
                RuntimeScriptError::new(processed.message, processed.stack, processed.cancelled)
                    .into()
            }
            CaughtError::Value(value) => ScriptError::Value(value_to_string(&value)),
        }
    }

    /// Retrieves and formats a pending JavaScript exception before QuickJS can overwrite it.
    pub(crate) fn process_js_error(&self, ctx: &Ctx<'_>, error: rquickjs::Error) -> ScriptError {
        let error = CaughtError::from_error(ctx, error);
        Self::process_caught_error(error, self.sourcemaps.as_ref())
    }
...
"#,
    ...
    #[tokio::test]
    async fn callback_error_processing_retrieves_pending_exception() {
        let engine = Engine::new().await.unwrap();
        let error_processor = engine.clone();

        let error = engine
            .with(move |ctx| {
                let error = ctx
                    .eval::<(), _>(
                        "function callback() { return missingCallbackValue; } callback();",
                    )
                    .unwrap_err();
                Ok(error_processor.process_js_error(&ctx, error).to_string())
            })
            .await
            .unwrap();
        ...
        assert!(
            error.contains("ReferenceError:")
                && error.contains("missingCallbackValue")
                && error.contains("is not defined"),
            "the JavaScript exception should replace the QuickJS wrapper, got: {error}"
        );
        assert!(
            error.contains("at callback"),
            "the JavaScript stack should be retained, got: {error}"
        );
        assert!(!error.contains("Exception generated by QuickJS"));
    }
