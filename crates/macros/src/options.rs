...
fn expand_struct(
    ...
    // When the struct also derives `FromJsObject`, keep `#[default]` on fields so the
    // derive can distinguish "optional with explicit default" from "required".
    // For other structs (no `FromJsObject` derive), strip `#[default]` as before to
    // avoid "unknown attribute" errors from built-in derives like `Clone` / `Debug`.
...
pub(crate) fn platform_not_from_attributes(
    ...
    for attribute in attributes {
        ...
        if let Some(not_value) = parsed_arguments.not_platform {
            if not_platform.is_some() {
                return Err(syn::Error::new_spanned(attribute, "duplicate `not` value"));
            }

            not_platform = Some(not_value);
        }
    }

    Ok(not_platform.map(|platform| platform.to_ascii_lowercase()))
}
