...
#[cfg(windows)]
use build_support::embed_windows_manifest;
...
fn embed_windows_icon() {
    println!("cargo:rerun-if-changed=actiona_run.rc");
    println!("cargo:rerun-if-changed=..\\core\\icons\\icon.ico");

    embed_resource::compile("actiona_run.rc", embed_resource::NONE)
        .manifest_optional()
        .expect("failed to compile actiona-run resources");
}
