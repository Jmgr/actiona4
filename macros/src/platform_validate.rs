...
fn derive_platform_validate_for_struct(
    ...
    for field in fields {
        ...
        let Some(allowed_platform) = platform_field_config.allowed_platform else {
            continue;
        };
        ...
        let presence_check = build_presence_check(
            field_name,
            &field.ty,
...
fn parse_platform_config_from_attributes(
    ...
    for attribute in attributes {
        ...
        if parsed_arguments.not_platform.is_some() {
            return Err(syn::Error::new_spanned(
                attribute,
                "unsupported `platform` key, expected `only`, `check`, `label`, or `nested`",
            ));
        }
