...
impl<'req, 'dev, 'win, W: wgpu::WindowHandle + 'win> PixelsBuilder<'req, 'dev, 'win, W> {
    ...
    async fn build_impl(self) -> Result<Pixels<'win>, Error> {
        ...
        // Prefer a non-Opaque alpha mode so transparent windows work with OS compositing.
        // Falls back to the first available mode (typically Opaque) if none are transparent.
