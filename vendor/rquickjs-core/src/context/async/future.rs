    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let mut lock = loop {
            ...
            if let LockState::Pending(ref mut fut) = &mut this.lock_state {
                ...
                // Use ptr::write to overwrite lock_state WITHOUT dropping the old value.
                // The old LockState::Pending's inner Lock<...> was already explicitly dropped
                // above via ManuallyDrop::drop(fut). Using a plain assignment would trigger
                // LockState::drop → ManuallyDrop::drop a second time, causing a double-free
                // of the EventListener's Box<InnerListener> when the lock was contended
                // (slow path, AcquireSlow path).
